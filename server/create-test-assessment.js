...
db.once('open', async function() {
  ...
  try {
    ...
    let user2TestAttempts = assessment.testAttempts.get(user2._id.toString());
    if (!user2TestAttempts) {
      user2TestAttempts = new Map();
      assessment.testAttempts.set(user2._id.toString(), user2TestAttempts);
    }
    
    // Get or create the attempts for this specific test
    const testId = tests[0]._id.toString();
    let testAttempt = user2TestAttempts.get(testId);
    if (!testAttempt) {
      testAttempt = { 
        attemptsUsed: 1, 
      ...
      user2TestAttempts.set(testId, testAttempt);
    }
    ...
    console.log('Assessment ID:', updatedAssessment._id);
    console.log('Test ID:', tests[0]._id);
